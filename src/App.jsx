...
const firebaseConfig = {
  apiKey: "AIzaSyC2EZ_RQw7j2y8RsfTvhQ1TXEoJsAWLnm8",
  authDomain: "quiniela-2026-37021.firebaseapp.com",
  projectId: "quiniela-2026-37021",
  storageBucket: "quiniela-2026-37021.firebasestorage.app",
  messagingSenderId: "175931037940",
  appId: "1:175931037940:web:b19d91d4d4f02c5d64a6a1"
};
const firebaseApp = initializeApp(firebaseConfig);
const db = getFirestore(firebaseApp);
const STORE_DOC = "quiniela/data";
...
export default function App() {
  ...
  const persist = async (newStore) => {
    setStore(newStore);
    setSaving(true);
    try {
      const [col, docId] = STORE_DOC.split("/");
      await setDoc(doc(db, col, docId), newStore);
    } catch(e) { console.error("Firebase save error:", e); }
    setSaving(false);
  };

  // Cargar y escuchar cambios en tiempo real desde Firebase
  useEffect(() => {
    const [col, docId] = STORE_DOC.split("/");
  ...
  const handleDeleteUser = async (name) => {
    ...
    await persist({ ...store, users: newUsers });
  ...
  const handleAdminSave = async () => {
    await persist({ ...store, realResults: adminResults });
